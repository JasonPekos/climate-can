...
pe_train <- pe_ts_full %>%
  ...
  select(GeoUID, Date, mean_temp_high, mean_temp_low, mean_pcp_high, mean_pcp_low, t, s, tot_prod)
...
pe_model <- pe_ts_full %>%
  ...
  select(GeoUID, Date, mean_temp_high, mean_pcp_high, mean_temp_low, mean_pcp_low, t, s, tot_prod) %>%
...
raw_geom_data_pe <- as_Spatial(tar_read(raw_geom_data_pe))
...
#Remove the row corresponding to missing region
row.rm =unique(pe_train$s[pe_train$GeoUID == 1101050])
w_pe = w_pe [-row.rm, -row.rm]
...
# Get the predictions
tot_prod_preds_high <- apply(M.final.pe_high$fit$samples$Y, 2, mean)
tot_prod_preds_low <- apply(M.final.pe_low$fit$samples$Y, 2, mean)

# Add the predictions with the pe_model dataframe
pe_pred_h <- c(pe_train$tot_prod, tot_prod_preds_high)
pe_pred_l <- c(pe_train$tot_prod, tot_prod_preds_low)
pe_model_pred <- cbind(pe_model, pe_pred_h, pe_pred_l) %>%
  mutate(diff = pe_pred_h - pe_pred_l)
